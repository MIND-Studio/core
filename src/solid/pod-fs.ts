...
/**
 * POSIX-shaped wrappers around the Solid LDP HTTP API. Ported from mind-os-v0;
 * promoted here so every app shares one implementation.
 *
 * Limits we accept (Solid-protocol-level, not ours to fix):
 *   - LDP PUT replaces the whole resource. Write is whole-file only.
 *   - No native move / rename. Use `rename` here = copy + unlink.
 *   - `deleteContainer` errors if non-empty; `rmrf` walks the tree first.
 *   - `readdir()` is one level deep.
 *
 * The active authed fetch (brokered shell fetch or local OIDC fetch) is
 * supplied by the owning {@link import("./create-client").SolidClient} via
 * `getFetch`, so callers never wire auth themselves.
 */
...
export interface PodFs {
  /** The active authed fetch (brokered or local). */
  podFetch(): typeof fetch;
  readdir(containerUrl: string): Promise<PodEntry[]>;
  readFileText(url: string): Promise<string>;
  readFileBlob(url: string): Promise<Blob>;
  writeFileText(url: string, contents: string, contentType?: string): Promise<void>;
  writeFileBlob(url: string, blob: Blob, contentType?: string): Promise<string>;
  unlink(url: string): Promise<void>;
  mkdir(url: string): Promise<string>;
  rmrf(url: string): Promise<void>;
  rename(fromUrl: string, toUrl: string, contentType?: string): Promise<void>;
}
...
/** Build a {@link PodFs} bound to a fetch resolver (brokered-or-local). */
export function createPodFs(getFetch: () => typeof fetch): PodFs {
  const authedFetch = getFetch;
  ...
  async function readdir(containerUrl: string): Promise<PodEntry[]> {
  ...
  async function readFileText(url: string): Promise<string> {
    const blob = await getFile(url, { fetch: authedFetch() });
    return await blob.text();
  }

  async function readFileBlob(url: string): Promise<Blob> {
  ...
  async function mkdir(url: string): Promise<string> {
   ...
   */
  async function rmrf(url: string): Promise<void> {
    if (!url.endsWith("/")) {
      await unlink(url);
      return;
    }
  ...
  async function rename(
    ...
    if (fromUrl.endsWith("/")) {
      throw new Error("rename: container rename not implemented");
    }
    const blob = await readFileBlob(fromUrl);
    const type = contentType ?? (blob.type || guessContentType(fromUrl));
    await writeFileBlob(toUrl, blob, type);
    await unlink(fromUrl);
  }

  return {
    podFetch: () => authedFetch(),
    readdir,
    readFileText,
    readFileBlob,
    writeFileText,
